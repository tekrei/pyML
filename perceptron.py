...
by M. Tim Jones
from https://github.com/mtimjones/perceptron
...
class Perceptron:
    ...
    def train(self,test, learning_rate, max_iter):
        ...
        while True:
            iteration_error = 0.0;
            print("Current iteration: ").format(current);
            for i in range(len(test)):
                desired_output = test[i][0] or test[i][1]
                output = self.feedforward(test[i])
                error = desired_output - output
                print("{} or {} = {} ({})").format(test[i][0], test[i][1], output, desired_output)
...
def main():
    ...
    print("initialized weights {} {} bias {}").format(perceptron.weights[0], perceptron.weights[1], perceptron.weights[2])
    # train the perceptron with test data, learning rate and maximum iteration count
    perceptron.train(test, 0.1, 10)
    print("final weights {} {} bias {}").format(perceptron.weights[0], perceptron.weights[1], perceptron.weights[2])
main()
